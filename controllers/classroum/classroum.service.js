...
module.exports.create = async (data) => {
  ...
  try {
    ...
    const _classrooms = schoolYear.classroomIds;
    const _classroom = _classrooms.find(c => c.name === data.name)
    if (_classroom) {
...
module.exports.note = async (id, pupilId, noteByPeriodId, data) => {
  try {
    ...
    const noteByPeriod = pupil.notesByPeriod.find((n) =>
      n._id.equals(noteByPeriodId)
    );
    if (!noteByPeriod)
      return handleError.errorConstructor(
        STATUS_CODE.NOT_FOUND,
    ...
    if (!c) return handleError.errorConstructor(STATUS_CODE.UNEXPECTED_ERROR);

    return handleError.errorConstructor(
      STATUS_CODE.SUCCESS,
      getCurrentObject(noteByPeriod.notes)
    );
  } catch (err) {
    console.log(err);
    return handleError.errorConstructor(STATUS_CODE.UNEXPECTED_ERROR);
...
module.exports.matter = async (id, data) => {
  try {
    ...
    return handleError.errorConstructor(
      STATUS_CODE.SUCCESS,
      getCurrentObject(classroom.matters)
    );
  } catch (err) {
...
module.exports.teacher = async (id, data) => {
  try {
    ...
    const school = await (
      await schoolService.getOne(classroom.schoolId)
    ).send.docs;
    ...
    if (!getObjectValue(teacherId, school.actors)) {
      return handleError.errorConstructor(
        STATUS_CODE.NOT_FOUND,
        null,
        "ce professeur n'existe pas dans l'établissement"
      );
    }
    ...
    if (teacher) {
      return handleError.errorConstructor(
        STATUS_CODE.DATA_EXIST,
        null,
        "ce professeur existe déjà dans la classe"
      );
    }
    ...
    const c = await utilsTools.save(
      classroom,
      getCurrentObject(classroom.teachers)
    );
...
module.exports.absence = async (id, data) => {
  try {
    ...
    return handleError.errorConstructor(
      STATUS_CODE.SUCCESS,
      getCurrentObject(classroom.absences)
    );
  } catch (error) {
...
module.exports.addAbsence = async (id, data) => {
  try {
    ...
    return handleError.errorConstructor(
      STATUS_CODE.SUCCESS,
      getObjectValue(data.absenceId, classroom.absences)
    );
  } catch (error) {
...
module.exports.pupil = async (id, data) => {
  try {
    ...
    if (data.role) {
      ...
      if (!_role) {
        return handleError.errorConstructor(
          STATUS_CODE.DATA_INCORRECT,
          null,
          "role incorect, utiliser " + PUPIL_ROLE.toString()
        );
      }
    }
    ...
    if (isEmpty(periods)) {
      return handleError.errorConstructor(
        STATUS_CODE.NOT_DATA,
        null,
        "aucune periode dans votre établissement"
      );
    }
...
module.exports.update = async (id, data) => {
  try {
    ...
    if (data?.principalId) {
      const school = (await schoolService.getOne(classroom.schoolId))?.send
        ?.docs;
      if (!getObjectValue(data.principalId, school.actors)) {
        return handleError.errorConstructor(
          STATUS_CODE.NOT_FOUND,
          null,
          "utilisateur n'existe pas dans le personnel"
        );
      }
    ...
    if (data?.totalPrice) {
      ...
      if (data.schoolYearsId) {
        schoolYear = school.schoolYears.find((d) =>
          d._id.equals(data.schoolYearsId)
        );
      } else {
      ...
      for (const d of deadlines) {
        d.price =
          (getObjectValue(d.periodId, schoolYear.deadlines).price / 100) *
          data.totalPrice;
      }
    }
      ...
      if (data.matterTeacherId) {
        if (!getObjectValue(data.matterTeacherId, school.actors)) {
          return handleError.errorConstructor(
            STATUS_CODE.NOT_FOUND,
            null,
            "l'établissement ne contient pas de professeur pour cette matière"
          );
        }
...
module.exports.updatePupil = async (id, data) => {
  try {
    ...
    if (data.lastname) pupil.lastname = data.lastname;

    if (data.firstname) pupil.firstname = data.firstname;

    if (data.birthCountry) pupil.birthCountry = data.birthCountry;

    if (data.oldSchool) pupil.oldSchool = data.oldSchool;

    if (data.birthday) pupil.birthday = data.birthday;

    if (data.pay) pupil.pay = data.pay;

    if (data.sanction) pupil.sanction = data.sanction;

    if (data.average) pupil.average = data.average;

    if (data.rank) pupil.rank = data.rank;

    if (data.complement != null) pupil.complement = data.complement;
    ...
    if (data.noteByPeriodId) {
      const noteByPeriod = getObjectValue(
        data.noteByPeriodId,
        pupil.notesByPeriod
      );
...
module.exports.updateAbsence = async (id, data) => {
  try {
    ...
    if (data.date) absence.date = data.date;

    if (data.timeNumber) absence.timeNumber = data.timeNumber;
    ...
    if (data.reason || data.justify != null) {
      ...
      if (data.reason) pupil.reason = data.reason;
      ...
      if (data.justify != null) {
        ...
        if (!_justify) {
          return handleError.errorConstructor(
            STATUS_CODE.DATA_INCORRECT,
            null,
            "boolean incorect. utiliser " + BOOL
          );
        }
  ...
  try {
    ...
    if (!theTeacher) {
      return handleError.errorConstructor(
        STATUS_CODE.NOT_FOUND,
        null,
        "ce professeur n'existe pas dans la liste des professeurs de la classe"
      );
    }
